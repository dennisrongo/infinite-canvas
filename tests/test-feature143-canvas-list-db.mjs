...
async function test7_verifyPersistence() {
  ...
  // Verify we still have our remaining canvases
...
async function test9_checkNoMockData() {
  ...
  // Check for common mock patterns in sidebar-related code
  const patterns = [
    'globalThis.devStore',
    'globalThis.mockDb',
    'mockData',
  ...
  for (const pattern of patterns) {
    try {
      const output = execSync(`grep -r "${pattern}" app/dashboard/page.tsx app/api/canvases/route.ts app/api/folders/route.ts 2>/dev/null || echo "NOT_FOUND"`, {
        encoding: 'utf8'
      });
      if (!output.includes('NOT_FOUND')) {
        console.log(`⚠ Warning: Found pattern "${pattern}" in source code`);
  ...
  console.log(`✓ No hardcoded or mock canvas list patterns detected`);
  ...
  if (canvases.length > 0) {
    ...
    if (canvas.id && typeof canvas.id === 'string' && canvas.id.length > 10) {
      console.log(`✓ Canvas has proper database ID format`);
    }
...
async function runTests() {
  ...
  try {
    ...
    const deletedCanvas = await test5_deleteCanvas(createdCanvases);
    await test6_verifyDisappearance(deletedCanvas);
    await test7_verifyPersistence();
    ...
    console.log('  ✓ No hardcoded or mock data detected');
